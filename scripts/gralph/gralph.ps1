...
function Get-TaskTouchesByIdYamlV1 {
  param([string]$Id)
  Invoke-YqWithEnv "GRALPH_TASK_ID" $Id '.tasks[] | select(.id == strenv(GRALPH_TASK_ID)) | .touches[]?'
}
...
function Run-ParallelAgentYamlV1 {
  ...
  $taskTitle = Get-TaskTitleByIdYamlV1 $TaskId
  $touches = (Get-TaskTouchesByIdYamlV1 $TaskId) -join ", "
...
"@
  ...
  Remove-Item $tmpfile -Force -ErrorAction SilentlyContinue
    if ($success) {
      # Revert tasks.yaml if modified by agent
      if (Test-Path (Join-Path $worktreeDir "tasks.yaml")) {
         cmd /c "git -C `"$worktreeDir`" reset HEAD tasks.yaml 2>nul"
         cmd /c "git -C `"$worktreeDir`" checkout -- tasks.yaml 2>nul"
      }
      ...
      # Check for meaningful changes (ignoring tasks.yaml and progress.txt)
      $changedFiles = (cmd /c "git -C `"$worktreeDir`" diff --name-only `"$($script:BASE_BRANCH)..HEAD`" 2>nul")
      $meaningfulChanges = $false
      if ($changedFiles) {
          $files = $changedFiles -split "`r?`n"
          foreach ($f in $files) {
              if ($f.Trim() -and $f -notmatch 'tasks\.yaml' -and $f -notmatch 'progress\.txt') {
    ...
    if (-not $commitCount -or [int]$commitCount -eq 0 -or -not $meaningfulChanges) {
      Add-Content -Path $LogFile -Value "[ERROR] Task failed: No meaningful code changes generated (only tasks.yaml/progress.txt or no commits)."
      "failed" | Set-Content -Path $StatusFile
      "0 0" | Set-Content -Path $OutputFile
